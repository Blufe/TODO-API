...
sub exist_word {
    my $self   = shift;
    my ($userid, $word, $type) = @_;
    my $table  = $TABLES->{"WORD_LIST"};
    my @values = ($word, $type, $userid);
    my @types  = (SQL_VARCHAR, SQL_VARCHAR, SQL_VARCHAR);
    my $exist  = 0;

    if (&connected($self)) {
	eval {
	    $exist = &mysql::exist_record($self->{"dbh"}, $table, 
					  "($table.word=?) AND ($table.type=?) AND ($table.userID=?)",
					  \@values, \@types);
	};
	if ($@) {
	    &error($self, 14, $@);
...
sub reg_word {
	...
	eval {
	    $w_id = &mysql::insert($self->{"dbh"}, $table, 
				   "$table.word, $table.type, $table.userID",
				   "?, ?, ?",
				   \@values, \@types);
	};
	if ($@) {
...
sub get_sentenceList {
    my $self   = shift;
    my ($userid, $words_lp, $date_b, $date_a, $finished, $order) = @_;
    my @words  = @$words_lp;
	...
	if ($order) {
	    if ($order eq "asc") {
		$where .= "ORDER BY $table.creation_date ASC, $table.sentenceID ASC";
	    } elsif ($order eq "desc") {
		$where .= "ORDER BY $table.creation_date DESC, $table.sentenceID DESC";
	    }
	}
	eval {
